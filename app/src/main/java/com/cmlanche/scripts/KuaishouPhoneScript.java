 ...
 */
public class KuaishouPhoneScript extends BaseScript {

    public KuaishouPhoneScript(AppInfo appInfo) { super(appInfo); }

    @Override
    protected void executeScript() {
        try {
            if (!isTargetPkg()) {
                Logger.i("快手[手机]：不在快手页面，等待中...");
                Utils.sleep(2000);
                return;
            }
