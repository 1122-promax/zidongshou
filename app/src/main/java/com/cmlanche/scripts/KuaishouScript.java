...
/**
 * 快手 - 模拟器模式
 * （同步自抖音手机脚本 DouyinPhoneScript）
 */
public class KuaishouScript extends BaseScript {

    public KuaishouScript(AppInfo appInfo) { super(appInfo); }

    @Override
    protected void executeScript() {
        try {
            if (!isTargetPkg()) {
                Logger.i("快手[模拟器]：不在快手页面，等待中...");
                Utils.sleep(2000);
                return;
            }
