...
public class DouyinFastScript extends BaseScript {
    ...
    public DouyinFastScript(AppInfo appInfo) { super(appInfo); }

    @Override
    protected void executeScript() {
        try {
            if (!isTargetPkg()) {
                Logger.i("抖音极速版[模拟器]：不在抖音页面，等待中...");
                Utils.sleep(2000);
                return;
            }
        ...
        for (int scan = 0; scan < 3 && !directHit && expandBtn == null; scan++) {
            ...
            for (OcrResult r : results) {
                ...
                if (PhoneVideoAnalyzer.isExpandText(txt)) {
                    expandBtn = r;
                    Logger.i("→ 找到展开文字: [" + txt + "] 区域=" + r.getRect().toShortString());
                }
            }

            if (!directHit && expandBtn == null) {
